...
class ContextSummarizer {
    ...
    async compressContext(context) {
        ...
        const targetOlderTurns = Math.max(1, Math.floor((this.config.maxContextLength - this.config.preserveRecentTurns) * this.config.compressionRatio));
        const selectedOlderTurns = this.selectMostRelevantTurns(olderTurns, relevanceScores, targetOlderTurns);
        // Generate summary for removed turns
        const removedTurns = olderTurns.filter(turn => !selectedOlderTurns.some(selected => selected.id === turn.id));
    ...
    applyCleanupPolicies(contexts) {
        ...
        for (const policy of this.expirationPolicies) {
            const expiredContexts = contexts.filter(context => this.isContextExpiredByPolicy(context, policy, now));
            ...
            contexts.splice(0, contexts.length, ...contexts.filter(context => !this.isContextExpiredByPolicy(context, policy, now)));
        }
        return {
            contextsRemoved,
            turnsRemoved,
    ...
    calculateTurnRelevance(turns, cumulativeContext) {
        ...
        for (const turn of turns) {
            ...
            const overallScore = (entityRelevance * this.config.entityImportanceWeight) +
    ...
    calculateEntityRelevance(turn, cumulativeContext) {
    ...
    calculateTopicRelevance(turn, cumulativeContext) {
        let relevanceScore = 0;
    ...
    calculateInteractionScore(turn) {
        let score = 0;
    ...
    selectImportantTurns(turns, relevanceScores) {
        return turns
            .filter(turn => {
    ...
    selectMostRelevantTurns(turns, relevanceScores, count) {
        return turns
            .map(turn => ({
            turn,
    ...
    generateSummaryText(turns, cumulativeContext) {
        if (turns.length === 0) {
    ...
    extractKeyPoints(turns, cumulativeContext) {
        const keyPoints = [];
    ...
    extractRelevantEntities(turns, cumulativeContext) {
    ...
    calculateSummaryConfidence(turns, relevanceScores) {
        if (turns.length === 0)
            return 0;
    ...
    extractKeyTopicsFromTurns(turns, cumulativeContext) {
    ...
    updateCumulativeContext(originalContext, remainingTurns) {
        ...
        for (const [entity, mentions] of originalContext.extractedEntities) {
            const relevantMentions = mentions.filter(mention => remainingTurnIds.has(mention.turnId));
            if (relevantMentions.length > 0) {
                filteredEntities.set(entity, relevantMentions);
            }
        }
        // Filter other context elements similarly
        const filteredDocuments = originalContext.documentContext.filter(doc => remainingTurns.some(turn => turn.query.documentIds && turn.query.documentIds.includes(doc.documentId)));
        const filteredTopics = originalContext.topicProgression.filter(topic => topic.queryIds.some(id => remainingTurns.some(turn => turn.query.id === id)));
        const filteredRelationships = originalContext.relationshipMap.filter(rel => remainingTurns.some(turn => turn.id === rel.turnId));
        return {
    ...
    isContextExpiredByPolicy(context, policy, now) {
    ...
    evaluateCondition(context, condition) {
        ...
        switch (condition) {
            ...
            case 'low_confidence':
                const avgConfidence = context.conversationThread.reduce((sum, turn) => sum + turn.response.confidence.overall, 0) / context.conversationThread.length;
...
//# sourceMappingURL=ContextSummarizer.js.map