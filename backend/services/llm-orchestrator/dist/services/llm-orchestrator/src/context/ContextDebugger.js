...
exports.ContextDebuggerService = void 0;
class ContextDebuggerService {
    constructor(config) {
     ...
     */
    recordOperation(sessionId, contextId, operation, input, output, executionTimeMs, errors = [], warnings = [], additionalDebugData = {}) {
        const session = this.debugSessions.get(sessionId);
     ...
     */
    diagnoseContext(context, _includePerformanceHistory = true) {
        const issues = [];
        const warnings = [];
        let healthScore = 100;
        ...
        const recommendations = this.generateDiagnosticRecommendations(context, healthScore, avgOperationTime, errorRate, storageSize);
        return {
    ...
    validateContextIntegrity(context) {
        ...
        const extractedEntityTypes = new Set();
     ...
     */
    traceQueryFlow(sessionId, queryId) {
        const session = this.debugSessions.get(sessionId);
        const allOperations = session?.operations || this.debugHistory.get(sessionId) || [];
        // Find operations related to this query
        const relatedOperations = allOperations.filter(op => JSON.stringify(op.input).includes(queryId) ||
    ...
    exportDebugData(sessionId) {
        const sessions = sessionId
            ? [this.debugSessions.get(sessionId)].filter(Boolean)
    ...
    generateDiagnosticRecommendations(context, healthScore, avgOperationTime, errorRate, storageSize) {
...
exports.ContextDebuggerService = ContextDebuggerService;
//# sourceMappingURL=ContextDebugger.js.map