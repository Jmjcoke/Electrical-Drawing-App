...
class ConversationContextService {
     ...
     */
    async getContextBySessionId(sessionId) {
        // First check cache
     ...
     */
    async addTurn(contextId, query, response, followUpDetected = false) {
     ...
     */
    async getRelevantContext(sessionId, currentQuery, maxTurns = 5) {
    ...
    async updateAccessMetadata(contextId) {
        const context = this.contextCache.get(contextId);
        if (context) {
    ...
    extractContextContributions(query, response) {
        const contributions = [];
    ...
    async updateCumulativeContext(current, turn) {
        const updated = {
    ...
    calculateRelevanceScore(turn, currentQuery) {
        let score = 0;
