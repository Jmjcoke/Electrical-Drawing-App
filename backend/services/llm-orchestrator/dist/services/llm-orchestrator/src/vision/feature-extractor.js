...
class FeatureExtractor {
     ...
     */
    async extractFeatures(regionBuffer, boundingBox, fullImageBuffer) {
        try {
            ...
            const [geometricFeatures, visualFeatures, topologicalFeatures, contextualFeatures, momentFeatures, houghFeatures, symbolFeatures] = await Promise.all([
            ...
            throw new symbol_detection_types_1.ImageProcessingError(`Feature extraction failed: ${error instanceof Error ? error.message : String(error)}`, { boundingBox });
        }
    }
    /**
     * Extract geometric features using OpenCV contour analysis
     */
    async extractGeometricFeatures(imageBuffer, boundingBox) {
        try {
            ...
            const mainContour = contours.reduce((largest, current) => current.area > largest.area ? current : largest);
    ...
    async extractVisualFeatures(imageBuffer) {
        try {
            ...
            const stdIntensity = Math.sqrt(((channel.max - channel.mean) ** 2 + (channel.min - channel.mean) ** 2) / 2) / 255;
     ...
     */
    async extractTopologicalFeatures(imageBuffer) {
        try {
     ...
     */
    async extractContextualFeatures(boundingBox, fullImageBuffer) {
        try {
            ...
            const proximityToEdge = Math.min(centerX, 1 - centerX, centerY, 1 - centerY);
            // Scale analysis
            ...
            if (fullImageBuffer) {
                const neighborhoodFeatures = await this.analyzeNeighborhood(fullImageBuffer, boundingBox);
     ...
     */
    async extractMomentFeatures(imageBuffer) {
        try {
            // Convert to binary image for moment calculation
            const binary = await (0, sharp_1.default)(imageBuffer)
                .threshold(128)
                .raw()
                .toBuffer();
     ...
     */
    async extractHoughFeatures(imageBuffer) {
        try {
     ...
     */
    async extractSymbolFeatures(imageBuffer, boundingBox) {
        try {
            // Extract contour points
            const contoursResult = await this.imageProcessor.extractContoursWithOpenCV(imageBuffer);
            const mainContour = contoursResult.contours.length > 0
                ? contoursResult.contours.reduce((largest, current) => current.area > largest.area ? current : largest)
                : null;
            const contourPoints = mainContour
                ? mainContour.points.map(p => ({ x: p.x, y: p.y }))
            ...
            const connectionPoints = await this.detectConnectionPoints(imageBuffer, contourPoints);
    ...
    calculateCentroidDistance(points, boundingBox) {
        const centroid = this.calculateCentroid(points);
        const boxCenter = {
            x: boundingBox.x + boundingBox.width / 2,
            y: boundingBox.y + boundingBox.height / 2,
        };
        const distance = Math.sqrt(Math.pow(centroid.x - boxCenter.x, 2) + Math.pow(centroid.y - boxCenter.y, 2));
    ...
    calculateConvexHullPerimeter(points) {
        if (points.length < 2)
            return 0;
        let perimeter = 0;
        for (let i = 0; i < points.length; i++) {
    ...
    calculateHorizontalSymmetry(points) {
        ...
        for (const point of points) {
            ...
            const distances = points.map(p => Math.sqrt(Math.pow(p.x - point.x, 2) + Math.pow(p.y - mirrorY, 2)));
            const minDistance = Math.min(...distances);
            symmetryScore += 1 / (1 + minDistance);
        }
        return symmetryScore / points.length;
    }
    calculateVerticalSymmetry(points) {
        ...
        for (const point of points) {
            ...
            const distances = points.map(p => Math.sqrt(Math.pow(p.x - mirrorX, 2) + Math.pow(p.y - point.y, 2)));
            const minDistance = Math.min(...distances);
            symmetryScore += 1 / (1 + minDistance);
        }
        return symmetryScore / points.length;
    }
    calculateConvexityDefects(points) {
    ...
    async calculateTextureFeatures(imageBuffer) {
        try {
    ...
    async detectCorners(imageBuffer) {
    ...
    async detectLineSegments(edgeBuffer) {
    ...
    async calculateEdgeDensity(edgeBuffer) {
        try {
            const image = (0, sharp_1.default)(edgeBuffer);
            const stats = await image.stats();
    ...
    getDefaultSymbolFeatures(boundingBox) {
        return {
    ...
    generateDefaultContourPoints(boundingBox) {
        return [
    ...
    async analyzeSkeleton(imageBuffer) {
        return {
            chainCode: [0, 1, 2, 3, 4, 5, 6, 7],
            length: 50,
        };
    }
    async detectHoles(imageBuffer) {
        return { count: 0 };
    }
    async analyzeConnections(imageBuffer) {
        return {
            endpoints: 2,
            junctions: 1,
            branchPoints: 0,
    ...
    async analyzeNeighborhood(fullImageBuffer, boundingBox) {
        return {
            density: 0.3,
            alignment: 0.5,
            isolation: 0.4,
    ...
    calculateImageMoments(binary, width, height) {
    ...
    async detectHoughLines(edgeBuffer) {
        return [
            { rho: 100, theta: 0, votes: 50 },
            { rho: 150, theta: Math.PI / 2, votes: 40 },
        ];
    }
    async detectHoughCircles(edgeBuffer) {
        return [];
    }
    analyzeLineOrientations(lines) {
        if (lines.length === 0) {
        ...
        return {
            ...
            perpendicular: lines.filter(line => Math.abs(line.theta - dominant - Math.PI / 2) < 0.1 ||
                Math.abs(line.theta - dominant + Math.PI / 2) < 0.1).length,
        };
    }
    async detectConnectionPoints(imageBuffer, contourPoints) {
    ...
    async estimateStrokeWidth(imageBuffer) {
