...
class ContextExtractor {
     ...
     */
    async extractContext(queryText, existingContext, documentIds) {
        ...
        try {
            ...
            const relevantDocuments = await this.buildDocumentReferences(queryText, entities, documentIds);
     ...
     */
    async extractEntities(queryText) {
        const entities = [];
     ...
     */
    async buildDocumentReferences(queryText, entities, documentIds) {
     ...
     */
    calculateDocumentRelevance(_queryText, entities, _documentId) {
     ...
     */
    calculateContextScore(entities, topics, documents, existingContext) {
     ...
     * Calculate extraction confidence
     */
    calculateExtractionConfidence(entities, topics) {
     ...
     */
    buildConversationFlow(_currentQuery, previousQueries, maxDepth = 5) {
        const nodes = [];
