...
class SuggestionEngine {
    ...
    async generateSuggestions(request) {
        ...
        try {
            ...
            suggestions.push(...templateSuggestions, ...contextSuggestions, ...completionSuggestions, ...popularSuggestions);
     ...
     */
    async generateTemplateSuggestions(request) {
        const suggestions = [];
        ...
        for (const [category, templates] of Object.entries(this.questionTemplates)) {
            for (const template of templates) {
                ...
                if (relevance > 0.3) {
                    ...
                    const filledTemplate = this.fillTemplate(template, request.currentContext || {});
                    suggestions.push({
                        text: filledTemplate,
     ...
     */
    async generateContextSuggestions(request) {
        const suggestions = [];
        // Analyze context for circuit types
        const circuitTypes = this.identifyCircuitTypes(request.currentContext || {});
        for (const circuitType of circuitTypes) {
            const patterns = this.contextPatterns[circuitType];
            if (patterns) {
     ...
     */
    async generateCompletionSuggestions(request) {
        const suggestions = [];
     ...
     */
    async generatePopularSuggestions(request) {
        const suggestions = [];
     ...
     */
    selectContextualComponent(context) {
     ...
     */
    calculateTemplateRelevance(partial, template) {
        return this.calculateStringRelevance(partial, template);
    }
    /**
     * Calculate context-based relevance
     */
    calculateContextRelevance(partial, suggestion, circuitType) {
     ...
     */
    calculateStringRelevance(partial, candidate) {
        ...
        const matchingWords = partialWords.filter(word => candidateWords.some(cWord => cWord.includes(word) || word.includes(cWord)));
        return Math.min(matchingWords.length / partialWords.length, 1.0) * 0.6;
    }
    /**
     * Suggest common electrical objects for completion
     */
    suggestCommonObjects() {
        const objects = [
     ...
     */
    rankSuggestions(suggestions, _request) {
        return suggestions
            .map(suggestion => ({
            ...suggestion,
     ...
     */
    updateStats(suggestions, processingTime) {
