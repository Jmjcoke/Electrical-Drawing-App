...
class ConfidenceScorer {
     ...
     */
    async calculateConfidence(symbol, imageBuffer, imageQuality, weights = {}) {
        try {
            const finalWeights = { ...this.defaultWeights, ...weights };
            // Calculate individual confidence factors
            const factors = await this.calculateConfidenceFactors(symbol, imageBuffer, imageQuality);
            // Calculate weighted confidence score
            const confidence = (factors.patternMatchScore * finalWeights.patternMatch) +
            ...
            throw new symbol_detection_types_1.SymbolDetectionError(`Confidence calculation failed: ${errorMessage}`, 'CONFIDENCE_CALCULATION_ERROR', { symbolId: symbol.id, symbolType: symbol.symbolType });
        }
    }
    /**
     * Calculate individual confidence factors
     */
    async calculateConfidenceFactors(symbol, imageBuffer, imageQuality) {
        const factors = {
     ...
     */
    calculatePatternMatchScore(symbol) {
     ...
     */
    calculateMLPredictionScore(symbol) {
        let score = symbol.confidence;
     ...
     */
    async calculateContextValidationScore(symbol, imageBuffer) {
     ...
     */
    calculateGeometricConsistencyScore(symbol) {
        if (!symbol.features) {
     ...
     */
    calculateSpatialCoherenceScore(symbol) {
     ...
     */
    async analyzeLocalContext(_symbol, _imageBuffer) {
        try {
     ...
     */
    checkElectricalCharacteristics(symbol) {
        let score = 0.5;
        if (!symbol.features)
            return score;
     ...
     */
    getExpectedAspectRatio(symbolType) {
        const aspectRatios = {
            'resistor': 3.0,
            'capacitor': 1.5,
            'inductor': 2.5,
     ...
     */
    getExpectedComplexity(symbolType) {
        const complexities = {
            'resistor': 0.3,
            'capacitor': 0.2,
            'inductor': 0.6,
     ...
     */
    calculateEnsembleConfidence(symbols) {
        if (symbols.length === 0)
            return 0;
        if (symbols.length === 1)
            return symbols[0].confidence;
