...
const feature_extractor_1 = require("../vision/feature-extractor");
const uuid_1 = require("uuid");
class MLClassifier {
    constructor() {
        this.models = null;
        this.isModelLoaded = false;
        this.modelVersion = 'v2.0';
        ...
        this.featureExtractor = new feature_extractor_1.FeatureExtractor();
        this.initializeModels();
    }
    /**
     * Classify symbols using TensorFlow.js neural networks
     */
    async classifySymbols(imageBuffer, existingSymbols = []) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new symbol_detection_types_1.MLClassificationError(`Neural network classification failed: ${errorMessage}`, { bufferSize: imageBuffer.length, existingSymbolsCount: existingSymbols.length });
        }
    }
    /**
     * Extract regions of interest for classification
     */
    async extractRegionsOfInterest(imageBuffer) {
        try {
            ...
            throw new symbol_detection_types_1.MLClassificationError(`Region extraction failed: ${error instanceof Error ? error.message : String(error)}`, { bufferSize: imageBuffer.length });
        }
    }
    /**
     * Find symbol candidates using computer vision techniques
     */
    async findSymbolCandidates(_imageBuffer) {
        try {
            ...
            throw new symbol_detection_types_1.MLClassificationError(`Candidate detection failed: ${error instanceof Error ? error.message : String(error)}`, {});
        }
    }
    /**
     * Validate if a region could contain a symbol
     */
    isValidSymbolRegion(boundingBox) {
        ...
        return (boundingBox.area >= minArea &&
     ...
     */
    async extractRegionImage(_imageBuffer, boundingBox) {
        try {
            ...
            throw new symbol_detection_types_1.MLClassificationError(`Region image extraction failed: ${error instanceof Error ? error.message : String(error)}`, { boundingBox });
        }
    }
    /**
     * Extract advanced features from region
     */
    async extractAdvancedFeatures(_regionImage, boundingBox) {
        try {
            ...
            throw new symbol_detection_types_1.MLClassificationError(`Advanced feature extraction failed: ${error instanceof Error ? error.message : String(error)}`, { boundingBox });
        }
    }
    /**
     * Classify a batch of regions using neural networks
     */
    async classifyRegionBatch(regions, fullImageBuffer) {
        if (!this.models) {
            throw new symbol_detection_types_1.MLClassificationError('Models not initialized', {});
        }
        try {
            const predictions = [];
            // Extract features for all regions in parallel
            const featureExtractionPromises = regions.map(async (region) => {
                const features = await this.featureExtractor.extractFeatures(region.imageData, region.boundingBox, fullImageBuffer);
            ...
            try {
                ...
                const [mainPredictions, geometricPredictions, visualPredictions] = await Promise.all([
                    this.models.mainClassifier.predict(batchTensor),
                    this.models.geometricClassifier.predict(tf.slice(batchTensor, [0, 0], [-1, 17])),
                    this.models.visualClassifier.predict(tf.slice(batchTensor, [0, 17], [-1, 12])),
                ]);
                // Combine predictions for ensemble
                const ensembleInput = tf.concat([mainPredictions, geometricPredictions, visualPredictions], 1);
                const ensemblePredictions = this.models.ensembleModel.predict(ensembleInput);
            ...
            throw new symbol_detection_types_1.MLClassificationError(`Batch neural network inference failed: ${error instanceof Error ? error.message : String(error)}`, { batchSize: regions.length });
        }
    }
    /**
     * Prepare feature tensor from advanced feature vector
     */
    prepareFeatureTensor(features) {
        ...
        const topologicalArray = Object.values(features.topological).flat();
     ...
     */
    normalizeFeatures(features, means, stds) {
     ...
     */
    createDetectedSymbol(region, prediction) {
     ...
     */
    async enhanceWithEnsembleMethod(existingSymbols, mlSymbols) {
        try {
            const enhanced = [];
            const processedExisting = new Set();
            // Process ML symbols first
            for (const mlSymbol of mlSymbols) {
                const overlappingExisting = existingSymbols.find(existing => this.calculateOverlap(existing.boundingBox, mlSymbol.boundingBox) > 0.3);
     ...
     */
    async createConsensusSymbol(patternSymbol, mlSymbol) {
        ...
        const weightedConfidence = (patternSymbol.confidence * patternWeight +
     ...
     */
    async applyEnsembleFilters(symbols) {
     ...
     */
    removeSpatialDuplicates(symbols) {
        const filtered = [];
        ...
        for (const symbol of sortedSymbols) {
            ...
            const overlapping = sortedSymbols.filter(other => !processed.has(other.id) &&
                other.id !== symbol.id &&
     ...
     * Apply electrical engineering validation rules
     */
    applyElectricalValidation(symbols) {
     ...
     */
    enhanceExistingSymbols(existingSymbols, mlSymbols) {
        const enhanced = [];
        // Add all ML-only symbols
        for (const mlSymbol of mlSymbols) {
            const hasOverlap = existingSymbols.some(existing => this.calculateOverlap(existing.boundingBox, mlSymbol.boundingBox) > 0.3);
            if (!hasOverlap) {
                enhanced.push(mlSymbol);
            }
        }
        ...
        for (const existing of existingSymbols) {
            const overlappingML = mlSymbols.find(ml => this.calculateOverlap(existing.boundingBox, ml.boundingBox) > 0.3);
            if (overlappingML) {
    ...
    calculateOverlap(box1, box2) {
        return coordinate_mapper_1.CoordinateMapper.calculateIoU(box1, box2);
    }
    /**
     ...
     */
    getSymbolDescription(symbolType) {
        const descriptions = {
     ...
     */
    generateMockContourPoints(boundingBox) {
        return [
    ...
    async initializeModels() {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new symbol_detection_types_1.MLClassificationError(`Failed to initialize TensorFlow.js models: ${errorMessage}`, { modelVersion: this.modelVersion });
        }
    }
    /**
     * Create main classifier neural network
     */
    async createMainClassifierModel() {
     ...
     */
    async createGeometricClassifierModel() {
        const model = tf.sequential({
            layers: [
     ...
     */
    async createVisualClassifierModel() {
        const model = tf.sequential({
            layers: [
    ...
    async warmUpModels() {
        ...
        try {
            ...
            const [mainResult, geometricResult, visualResult, ensembleResult] = await Promise.all([
                this.models.mainClassifier.predict(dummyMainFeatures),
                this.models.geometricClassifier.predict(dummyGeometricFeatures),
                this.models.visualClassifier.predict(dummyVisualFeatures),
                this.models.ensembleModel.predict(dummyEnsembleFeatures),
            ]);
            // Clean up tensors
    ...
    getPerformanceMetrics() {
        return {
            ...
            averageInferenceTime: 50, // milliseconds (estimated)
            supportedBatchSize: 8,
     ...
     */
    setMemoryGrowth(enabled) {
        tf.env().set('WEBGL_DELETE_TEXTURE_THRESHOLD', enabled ? 1 : -1);
    }
}
exports.MLClassifier = MLClassifier;
//# sourceMappingURL=ml-classifier.js.map