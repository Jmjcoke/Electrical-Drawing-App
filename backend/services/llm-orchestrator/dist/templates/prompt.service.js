...
{{additional_instructions}}`,
    ...
    validateTemplate(template) {
        ...
        const undeclaredVariables = templateVariables.filter(variable => !template.variables.includes(variable));
        if (undeclaredVariables.length > 0) {
            throw new Error(`Template '${template.name}' contains undeclared variables: ${undeclaredVariables.join(', ')}`);
        }
    }
    interpolateVariables(template, variables) {
        ...
        if (remainingVariables.length > 0) {
            throw new Error(`Template contains unreplaced variables: ${remainingVariables.join(', ')}`);
        }
        return result;
    }
