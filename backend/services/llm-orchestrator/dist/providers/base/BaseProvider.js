...
class BaseProvider {
    ...
    validateConfig(config) {
        try {
            ...
            throw new LLMProvider_interface_1.ConfigurationError(`Configuration validation failed: ${errorMessage}`, this.name, errorCause);
        }
    }
    /**
     * Protected method for executing analysis with common error handling
     */
    async executeAnalysis(analysisFunction, options) {
        ...
        try {
            ...
            const response = await this.withTimeout(analysisFunction(), options?.timeout || this.config.timeout || 30000);
            // Update metrics
            metrics = {
                ...metrics,
                endTime: Date.now(),
    ...
    validateProviderSpecificConfig(_config) {
        return true;
    }
    /**
    ...
    checkRateLimit() {
        ...
        if (this.currentRateLimit.requestsRemaining <= 0) {
            ...
            throw new LLMProvider_interface_1.RateLimitError(`Rate limit exceeded. Retry after ${retryAfter} seconds`, this.name, retryAfter);
        }
    }
    /**
     ...
     */
    updateRateLimitAfterRequest(tokensUsed) {
    ...
    handleError(error) {
        ...
        if (error.status >= 400 && error.status < 500) {
            return new LLMProvider_interface_1.ConfigurationError(`Client error: ${error.message}`, this.name, error);
        }
        // Default to analysis error
        return new LLMProvider_interface_1.AnalysisError(`Analysis failed: ${error.message}`, this.name, error);
    }
    /**
