...
class GeminiProvider extends BaseProvider_1.BaseProvider {
        ...
        return this.executeAnalysis(async () => {
            ...
            try {
                ...
                if (response.promptFeedback?.blockReason) {
                    throw new LLMProvider_interface_1.AnalysisError(`Content blocked by safety filters: ${response.promptFeedback.blockReason}`, this.name);
                }
                // Extract response content
                const text = response.text();
                if (!text) {
                    throw new LLMProvider_interface_1.AnalysisError('No text content received from Gemini', this.name);
                }
                // Calculate confidence score based on safety ratings and finish reason
                const confidence = this.calculateConfidenceScore(response.candidates?.[0]?.safetyRatings || [], response.candidates?.[0]?.finishReason || 'STOP');
    ...
    async healthCheck() {
        try {
            ...
            const testImage = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAI9jU77yQAAAABJRU5ErkJggg==', 'base64');
            const result = await this.model.generateContent({
                contents: [{
                        role: 'user',
                        parts: [
    ...
    initializeClient() {
        try {
            ...
            throw new LLMProvider_interface_1.ConfigurationError(`Failed to initialize Gemini client: ${error}`, this.name, error instanceof Error ? error : undefined);
        }
    }
    /**
     * Validates Gemini-specific configuration
    ...
    validateGeminiConfig(config) {
        if (config.model !== 'gemini-pro-vision') {
            throw new LLMProvider_interface_1.ConfigurationError('Only gemini-pro-vision model is supported', this.name);
        }
    ...
    validateAnalysisInputs(image, prompt) {
        if (!image || image.length === 0) {
            throw new LLMProvider_interface_1.AnalysisError('Image buffer is required and cannot be empty', this.name);
        }
        if (!prompt || prompt.trim().length === 0) {
            throw new LLMProvider_interface_1.AnalysisError('Prompt is required and cannot be empty', this.name);
        }
     ...
     */
    calculateConfidenceScore(safetyRatings, finishReason) {
    ...
    handleGeminiError(error) {
        ...
        if (error?.message) {
            ...
            if (message.includes('safety') || message.includes('blocked')) {
                return new LLMProvider_interface_1.AnalysisError(`Content safety violation: ${error.message}`, this.name, error);
            }
            if (message.includes('timeout')) {
                return new LLMProvider_interface_1.AnalysisError(`Request timeout: ${error.message}`, this.name, error);
            }
        }
        // Default error handling
        return new LLMProvider_interface_1.AnalysisError(`Gemini analysis failed: ${error?.message || error}`, this.name, error);
    }
}
