...
export interface SymbolTemplate {
  ...
  priority: number; // Higher priority symbols are matched first
  ensembleWeight: number; // Weight in ensemble matching (0-1)
}
...
export interface SymbolVariant {
  ...
  templateImage: Buffer;
  usage: string;
  confidence: number; // base confidence for this variant
  features: SymbolFeatures; // variant-specific features
  keyPoints: KeyPoint[]; // rotation/scale invariant keypoints
}
...
export interface SymbolFeatures {
  ...
  colorProfile?: number[];
  invariantMoments: number[]; // Hu moments for rotation/scale invariance
  keyPoints: KeyPoint[]; // SIFT/ORB keypoints
  descriptors: number[][]; // Feature descriptors for matching
}
...
export class SymbolLibrary {
  ...
  private highPriorityTemplates: SymbolTemplate[] = [];
  private ensembleTemplates: SymbolTemplate[] = [];
  private isInitialized = false;
  private templateSize = 200; // Standard template size
  ...
  private async initializeLibrary(): Promise<void> {
    try {
      ...
      throw new ImageProcessingError(
        `Failed to initialize symbol library: ${errorMessage}`,
        {}
      );
    }
  }
   ...
   */
  private getComprehensiveDrawingSpecs(): DrawingSpec[] {
    return [
   ...
   */
  private async createAdvancedSymbolTemplate(spec: DrawingSpec): Promise<void> {
    try {
      const template: SymbolTemplate = {
        id: uuidv4(),
        symbolType: spec.name as ElectricalSymbolType,
        symbolCategory: this.getSymbolCategory(spec.name as ElectricalSymbolType),
      ...
      for (const rotation of rotations) {
        for (const scale of scales) {
          if (rotation === 0 && scale === 1.0) continue; // Skip base template
   ...
   */
  private async generateSymbolImage(
    spec: DrawingSpec, 
    rotation: number, 
    scale: number
  ): Promise<{ buffer: Buffer; features: SymbolFeatures }> {
  ...
  private async extractAdvancedFeatures(
    ...
    const descriptors: number[][] = keyPoints.map(() => 
      Array.from({ length: 128 }, (_, i) => Math.sin(i * 0.1 + rotation * 0.01) * 255)
    );
   ...
   */
  private calculateVariantBaseConfidence(rotation: number, scale: number): number {
    // Higher confidence for common rotations and scales
    let confidence = 0.5;
    
    // Rotation adjustments
    if (rotation % 90 === 0) confidence += 0.2; // Right angles
    if (rotation === 0 || rotation === 180) confidence += 0.1; // Standard orientations
    
    // Scale adjustments  
    if (scale >= 0.8 && scale <= 1.2) confidence += 0.2; // Common scales
    if (scale === 1.0) confidence += 0.1; // Perfect scale
   ...
   */
  private createSymbolMetadata(spec: DrawingSpec): SymbolMetadata {
    const symbolType = spec.name as ElectricalSymbolType;
    
    // Define metadata based on symbol type
    const metadataMap: Record<string, Partial<SymbolMetadata>> = {
      resistor: {
   ...
   * Organize templates for efficient matching
   */
  private organizeTemplatesForMatching(): void {
