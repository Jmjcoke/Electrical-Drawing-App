...
export class CircuitBreaker {
  ...
  public async execute<T>(operation: () => Promise<T>): Promise<T> {
    if (!this.canExecute()) {
      throw new CircuitBreakerOpenError(
        `Circuit breaker is ${this.state}. Next retry at: ${this.nextRetryTime?.toISOString()}`,
        this.name,
        this.getMetrics()
      );
    }
   ...
   */
  public getRequestHistory(): ReadonlyArray<RequestResult> {
    this.cleanOldRequests();
   ...
   */
  private onSuccess(result: RequestResult): void {
   ...
   */
  private onFailure(result: RequestResult): void {
    this.recordRequest(result);
    this.failures++;
...
export class CircuitBreakerFactory {
   ...
   */
  public static createForLLMProvider(name: string): CircuitBreaker {
   ...
   */
  public static createAggressive(name: string): CircuitBreaker {
    const config: CircuitBreakerConfig = {
   ...
   */
  public static createPermissive(name: string): CircuitBreaker {
    const config: CircuitBreakerConfig = {
...
export class CircuitBreakerManager {
   ...
   */
  public getCircuitBreaker(providerName: string, factory?: () => CircuitBreaker): CircuitBreaker {
   ...
   */
  public removeCircuitBreaker(providerName: string): boolean {
