...
import { LLMResponse } from '../providers/base/LLMProvider.interface';
...
export class PerformanceMonitor {
  ...
  private requestHistory: EnsembleRequestRecord[] = [];
   ...
   */
  public recordEnsembleRequest(
    request: EnsembleRequestContext,
    response: EnsembleResponse,
    duration: number
  ): void {
    const record: EnsembleRequestRecord = {
      id: response.id,
      timestamp: new Date(),
      duration,
      success: true,
   ...
   */
  public checkSLACompliance(): SLACompliance {
   ...
   */
  private checkSLAViolations(record: EnsembleRequestRecord): void {
   ...
   */
  private createAlert(violation: Omit<SLAViolation, 'timestamp' | 'duration'>): void {
   ...
   */
  private calculateEnsembleMetrics(requests: EnsembleRequestRecord[]): EnsembleMetrics {
   ...
   */
  private calculateProviderMetrics(requests: EnsembleRequestRecord[]): Map<string, ProviderMetrics> {
    const providerStats = new Map<string, ProviderMetrics>();
    ...
    for (const request of requests) {
      for (const response of request.individualResponses) {
        ...
        (stats as any).requests = currentRequests + 1;
        (stats as any).successes = stats.successes + 1;
        (stats as any).averageResponseTime = (stats.averageResponseTime * currentRequests + response.responseTime) / stats.requests;
        (stats as any).averageConfidence = (stats.averageConfidence * currentRequests + response.confidence) / stats.requests;
        (stats as any).totalCost = stats.totalCost + (response.tokensUsed * 0.001); // Simplified cost calculation
      }
    }
  ...
  private calculateSystemMetrics(): SystemMetrics {
    ...
    return {
      ...
      activeConnections: this.requestHistory.filter(r => 
   ...
   */
  private calculateSLACompliance(requests: EnsembleRequestRecord[]): SLACompliance {
    if (requests.length === 0) {
      return {
    ...
    const responseTimeCompliance = requests.filter(r => 
      r.duration <= thresholds.maxEnsembleResponseTime
    ).length / requests.length;

    const successRateCompliance = requests.filter(r => r.success).length / requests.length;

    const costCompliance = requests.filter(r => 
      r.cost <= thresholds.maxCostPerRequest
    ).length / requests.length;

    const confidenceCompliance = requests.filter(r => 
      r.confidence >= thresholds.minConfidenceScore
    ).length / requests.length;

    const efficiencyCompliance = requests.filter(r => 
      (1 - r.parallelEfficiency) <= thresholds.maxParallelEfficiencyLoss
    ).length / requests.length;
   ...
   * Helper methods
   */
  private getRecentRequests(windowMs: number): EnsembleRequestRecord[] {
  ...
  private determineSeverity(actual: number, threshold: number): ViolationSeverity {
  ...
  private calculateThroughput(requests: EnsembleRequestRecord[]): number {
  ...
  private calculateCostEfficiency(requests: EnsembleRequestRecord[]): number {
  ...
  private calculateResponseTimeDistribution(requests: EnsembleRequestRecord[]): number[] {
  ...
  private calculateErrorRateByProvider(requests: EnsembleRequestRecord[]): Record<string, number> {
  ...
  private calculateTimeSeries(metrics: PerformanceMetrics[], valueExtractor: (m: PerformanceMetrics) => number): TimeSeries {
  ...
  private getProviderComparison(metrics: PerformanceMetrics): ProviderComparison[] {
  ...
  private determineSystemStatus(metrics: PerformanceMetrics): 'healthy' | 'degraded' | 'critical' {
    const compliance = metrics.slaCompliance.overallCompliance;
    const errorRate = metrics.systemMetrics.errorRate;
    
    if (compliance < 0.8 || errorRate > 0.1) return 'critical';
    if (compliance < 0.95 || errorRate > 0.05) return 'degraded';
    return 'healthy';
  }

  private groupAlertsByType(alerts: Alert[]): Record<SLAViolationType, number> {
    const groups: Record<SLAViolationType, number> = {
  ...
  private groupAlertsBySeverity(alerts: Alert[]): Record<ViolationSeverity, number> {
    const groups: Record<ViolationSeverity, number> = {
  ...
  private identifyProviderStrengths(metrics: ProviderMetrics): string[] {
    const strengths: string[] = [];
    
    if (metrics.averageResponseTime < 2000) strengths.push('Fast response times');
    if (metrics.availability > 0.99) strengths.push('High availability');
    if (metrics.averageConfidence > 0.8) strengths.push('High confidence responses');
    if (metrics.circuitBreakerTrips === 0) strengths.push('Reliable service');
    
    return strengths;
  }

  private identifyProviderWeaknesses(metrics: ProviderMetrics): string[] {
    const weaknesses: string[] = [];
    
    if (metrics.averageResponseTime > 5000) weaknesses.push('Slow response times');
    if (metrics.availability < 0.95) weaknesses.push('Availability issues');
    if (metrics.averageConfidence < 0.6) weaknesses.push('Low confidence responses');
    if (metrics.circuitBreakerTrips > 0) weaknesses.push('Reliability concerns');
...
interface EnsembleRequestRecord {
  id: string;
  timestamp: Date;
  duration: number;
  success: boolean;
