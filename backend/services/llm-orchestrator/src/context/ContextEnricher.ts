...
export class ContextEnricher {
  private readonly recencyDecayFactor = 0.95;

  /**
   * Build cumulative knowledge from conversation turns
   */
  public buildCumulativeContext(turns: ConversationTurn[]): CumulativeContext {
  ...
  public async retrieveRelevantContext(
    ...
    const candidateTurns = this.filterCandidateTurns(
      context.conversationThread,
      request.maxContextTurns
    );

    const relevanceScores = await Promise.all(
      candidateTurns.map(turn => this.calculateRelevanceScore(
        request.currentQuery,
        turn,
        context.conversationThread.indexOf(turn)
      ))
    );
  ...
  public async enhanceQueryWithContext(
    query: string,
    relevantContext: ContextRelevanceScore[],
   ...
   */
  public mergeRelatedContexts(contexts: ConversationContext[]): ConversationContext {
   ...
   */
  public validateContext(context: ConversationContext): {
    isValid: boolean;
  ...
  private extractAndMergeEntities(turns: ConversationTurn[]): Map<string, EntityMention[]> {
  ...
  private extractEntitiesFromText(text: string, turnId: string): EntityMention[] {
  ...
  private buildDocumentContext(turns: ConversationTurn[]): DocumentContextSummary[] {
  ...
  private buildTopicProgression(turns: ConversationTurn[]): TopicNode[] {
  ...
  private extractTopics(text: string): string[] {
    ...
    if (/circuit|schematic|diagram/i.test(text)) topics.push('circuit_analysis');
    if (/component|part|element/i.test(text)) topics.push('component_identification');
    if (/voltage|current|power|resistance/i.test(text)) topics.push('electrical_properties');
    if (/safety|hazard|warning/i.test(text)) topics.push('safety_analysis');
    if (/specification|datasheet|rating/i.test(text)) topics.push('specifications');
  ...
  private buildRelationshipMap(turns: ConversationTurn[], entities: Map<string, EntityMention[]>): EntityRelationship[] {
  ...
  private filterCandidateTurns(turns: ConversationTurn[], maxTurns: number): ConversationTurn[] {
  ...
  private calculateSemanticSimilarity(query: string, turn: ConversationTurn): number {
    ...
    const commonWords = queryWords.filter(word => 
      turnWords.some(turnWord => turnWord.includes(word) || word.includes(turnWord))
    );
  ...
  private determineContextType(turn: ConversationTurn): ContextType {
    ...
    if (text.includes('how') || text.includes('why')) return 'insight';
    if (text.includes('connect') || text.includes('relate')) return 'relationship';
  ...
  private calculateEnhancementConfidence(contextSources: ContextSource[]): number {
  ...
  private mergeConversationTurns(contexts: ConversationContext[]): ConversationTurn[] {
  ...
  private mergeCumulativeContexts(contexts: CumulativeContext[]): CumulativeContext {
    const mergedEntities = new Map<string, EntityMention[]>();
    const mergedDocuments: DocumentContextSummary[] = [];
    const mergedTopics: TopicNode[] = [];
    const mergedInsights: string[] = [];
    const mergedRelationships: EntityRelationship[] = [];
