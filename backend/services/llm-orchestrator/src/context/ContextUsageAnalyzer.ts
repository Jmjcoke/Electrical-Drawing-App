...
export class ContextUsageAnalyzerService {
  private readonly config: ContextUsageAnalyzerConfig;
  private readonly usageData = new Map<string, ContextUsageAnalytics>();
  ...
  analyzeUsagePatterns(): UsagePattern[] {
    ...
    for (const [sessionId, usage] of this.usageData.entries()) {
      ...
      if (followUpRatio > 0.6) patternKey = 'high_followup';
      else if (followUpRatio > 0.3) patternKey = 'medium_followup';
      ...
      if (retrievalRatio > 0.8) retrievalPattern = 'high_retrieval';
      else if (retrievalRatio > 0.5) retrievalPattern = 'medium_retrieval';
      ...
      if (durationMinutes > 60) durationPattern = 'long_session';
      else if (durationMinutes > 20) durationPattern = 'medium_session';
  ...
  private updatePatternStats(
    patterns: Map<string, { count: number; performanceTimes: number[]; successCount: number }>,
    patternKey: string,
    sessionMetrics: ContextMetrics[],
    isSuccess: boolean
  ): void {
    ...
    const relevantMetrics = sessionMetrics.filter(m => 
      ['retrieval_time_ms', 'context_enhancement_time_ms'].includes(m.metricType)
    );
  ...
  private generatePatternRecommendation(
    pattern: string,
    stats: { count: number; performanceTimes: number[]; successCount: number }
  ): string {
