...
export class QueryOptimizer {
  ...
  private optimizationCache: Map<string, { result: QueryOptimizationResult; timestamp: number }>;
  ...
  async optimizeForProviders(
    ...
    try {
      ...
      const providerEnhancements: Record<string, QueryEnhancement> = {};

      for (const [providerId, providerConfig] of Object.entries(this.providerConfigs)) {
        const enhancement = await this.optimizeForProvider(
          queryText,
          intent,
          context,
          providerId,
  ...
  private buildBasePrompt(
    ...
    if (template.requiredInfo && template.requiredInfo.length > 0) {
      ...
      prompt += template.requiredInfo.map((info: string) => `- ${info}`).join('\n');
    }
   ...
   */
  private buildTechnicalContext(context: ContextExtractionResult, _providerConfig: any): string[] {
   ...
   */
  private getProviderParameters(intent: QueryIntent, providerConfig: any): Record<string, any> {
    const baseParams = {
  ...
  private calculateCostEstimation(
    ...
    for (const [providerId, prompt] of Object.entries(optimizedPrompts)) {
      const providerConfig = this.providerConfigs[providerId as keyof typeof this.providerConfigs];
   ...
   */
  private getCachedOptimization(cacheKey: string): QueryOptimizationResult | null {
   ...
   */
  private cacheOptimization(cacheKey: string, result: QueryOptimizationResult): void {
   ...
   */
  private createFallbackOptimization(queryText: string, _intent: QueryIntent): QueryOptimizationResult {
   ...
   */
  private updateStats(complexityScore: number, processingTime: number): void {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const testIntent: QueryIntent = {
        type: 'component_identification',
        confidence: 0.8,
        reasoning: 'test'
      };
      
      const testContext: ContextExtractionResult = {
        entities: [],
        topics: [],
        relevantDocuments: [],
        contextScore: 0.5,
