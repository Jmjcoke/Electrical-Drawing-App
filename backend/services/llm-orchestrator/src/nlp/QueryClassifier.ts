...
interface ClassificationConfig {
  ...
  readonly scoringWeights: {
    ...
    readonly scoreNormalizationBoost: number;
  };
}
...
export class QueryClassifier {
   ...
   */
  async classifyIntent(queryText: string): Promise<ClassificationResult> {
    ...
    try {
      ...
      const sortedIntents = Object.entries(scores)
        .map(([type, confidence]) => ({
          ...
          confidence,
          subcategory: this.determineSubcategory(type as QueryIntent['type'], normalizedQuery),
          reasoning: this.generateReasoning(type as QueryIntent['type'], normalizedQuery, confidence)
        }))
   ...
   */
  private calculateIntentScores(normalizedQuery: string): Record<QueryIntent['type'], number> {
    ...
    if (maxScore > 1.0) {
      Object.keys(scores).forEach(key => {
        scores[key as QueryIntent['type']] = Math.min(scores[key as QueryIntent['type']] / maxScore, 1.0);
      });
    } else if (maxScore > 0) {
      // If all scores are below 1.0, boost the highest one
      const highestKey = Object.entries(scores).reduce((a, b) => scores[a[0] as QueryIntent['type']] > scores[b[0] as QueryIntent['type']] ? a : b)[0] as QueryIntent['type'];
    ...
    if (sortedScores[0][1] > sortedScores[1][1] + 0.3) {
      scores[sortedScores[0][0] as QueryIntent['type']] = Math.min(scores[sortedScores[0][0] as QueryIntent['type']] + weights.dominantIntentBoost, 1.0);
    }
   ...
   */
  private countKeywordMatches(queryWords: string[], keywords: string[]): number {
   ...
   */
  private determineSubcategory(intentType: QueryIntent['type'], normalizedQuery: string): string {
    switch (intentType) {
      case 'component_identification':
        if (/passive/.test(normalizedQuery)) return 'passive-component';
        if (/active/.test(normalizedQuery)) return 'active-component';
        if (/ic|chip|integrated/.test(normalizedQuery)) return 'integrated-circuit';
        return 'general-component';

      case 'schematic_analysis':
        if (/power|supply/.test(normalizedQuery)) return 'power-analysis';
        if (/signal|ac|frequency/.test(normalizedQuery)) return 'signal-analysis';
        if (/dc|static/.test(normalizedQuery)) return 'dc-analysis';
        return 'general-analysis';

      case 'general_question':
        if (/how.*work/.test(normalizedQuery)) return 'operation-explanation';
        if (/what.*purpose|function/.test(normalizedQuery)) return 'purpose-explanation';
        if (/difference|compare/.test(normalizedQuery)) return 'comparison';
   ...
   */
  private updateStats(intent: QueryIntent, processingTime: number): void {
   ...
   */
  private initializeTrainingData(): ClassificationTrainingData[] {
    return [
