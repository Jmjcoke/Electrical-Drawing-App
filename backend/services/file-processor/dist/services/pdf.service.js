...
export class PdfService {
    constructor(redisUrl, baseStoragePath) {
     ...
     */
    async convertPdfToImages(documentId, sessionId, pdfBuffer, originalFilename, options = {}, progressCallback) {
        ...
        try {
            ...
            const outputDir = path.join(this.baseStoragePath, 'sessions', sessionId, 'converted', documentId);
     ...
     */
    async cacheResult(cacheKey, entry) {
        try {
            await this.redis.setex(cacheKey, 24 * 60 * 60, JSON.stringify(entry));
     ...
     */
    generateCacheKey(documentId, checksum) {
        ...
        try {
            ...
            for (const key of keys) {
                ...
                if (cached) {
                    ...
                    if (entry.imagePaths.some((path) => path.includes(sessionId))) {
     ...
     */
    async validatePdfBuffer(pdfBuffer, filename) {
        try {
            ...
            const validationResult = await validationService.validatePDF(mockFile);
            return {
     ...
     */
    async attemptFallbackConversion(pdfBuffer, originalOptions, documentId) {
    ...
    categorizeError(error) {
        ...
        if (message.includes('page') && message.includes('not found'))
            return 'pdf_empty';
        // System errors
        ...
        if (message.includes('poppler') || message.includes('pdftocairo'))
            return 'poppler_error';
        if (message.includes('buffer') || message.includes('data'))
            return 'buffer_error';
        return 'unknown';
    }
    /**
