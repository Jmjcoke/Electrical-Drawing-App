...
import { documentDatabase } from '../utils/conversion.js';
class FileService {
    constructor() {
    ...
    async processFile(filePath, originalFile, sessionId, documentId, progressCallback) {
        try {
            ...
            if (originalFile.mimetype === 'application/pdf') {
                ...
                const webSocketProgressCallback = (progress) => {
                ...
                const conversionResult = await this.pdfService.convertPdfToImages(documentId, sessionId, pdfBuffer, originalFile.originalname, { dpi: 300, format: 'png', quality: 95 }, webSocketProgressCallback);
                if (conversionResult.success) {
                    // Store converted images metadata
                    await storageService.storeConvertedImages(documentId, sessionId, conversionResult.imagePaths);
